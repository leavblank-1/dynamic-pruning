...
def generate_sankey_from_pkl_with_mean_probs(pkl_file_path):
    """
    从 .pkl 文件加载数据，并生成 CIFAR-10 类别在各层决策头选择的桑基图。
    流量宽度使用 mean_action_probs（平均行动概率）。
    ...
    for i, layer_name in enumerate(layer_names):
        ...
        for class_id in range(len(cifar10_classes)):
            ...
            # 遍历所有决策头，为每个决策头创建链接
            for action_index in range(len(class_data['mean_action_probs'])): # 注意这里遍历的是 mean_action_probs
                mean_action_prob = class_data['mean_action_probs'][action_index]
                action_name = action_names[action_index]

                # 只有当平均概率大于一个非常小的值时才创建链接，避免绘制几乎为零的流量
                # 例如，只绘制概率大于0.001的链接，可以根据需要调整
                if mean_action_prob > 0.001: # 设置一个阈值来过滤微弱的流量，使图更清晰
                    # 创建中间节点：表示当前层选择的决策头
                    ...
                    sankey_links_value.append(mean_action_prob) # **流量值使用 mean_action_prob**
                    sankey_links_label.append(
                        f'{cifar10_classes[class_id]} 在 {layer_name} '
                        f'流向 {action_name} (平均概率: {mean_action_prob:.3f})'
                    )
                    ...
                    if i < len(layer_names) - 1:
                        ...
                        sankey_links_value.append(mean_action_prob) # **流量值继续使用 mean_action_prob**
                        sankey_links_label.append(
                            f'{action_name} 引导 {cifar10_classes[class_id]} 进入 {next_layer_name} '
                            f'(流量: {mean_action_prob:.3f})'
                        )
    print("桑基图链接构建完成。")
    ...
    fig.update_layout(
        title_text="<b>CIFAR-10 类别在各层决策头选择的桑基图</b><br>"
                   "（图例：节点名称即是图例，流量宽度表示平均行动概率）", # 更新图例说明
        font_size=10
    )
...
if __name__ == "__main__":
    ...
    # 请替换为你的实际 .pkl 文件路径
    # 例如：pkl_file = r"C:\Users\Lenovo\Desktop\github\my_pruning\logs\finetune-decision-5\cifar10-resnet56\sparsity-0.40\action_probs_by_class.pkl"

    generate_sankey_from_pkl_with_mean_probs(pkl_file)